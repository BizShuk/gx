...
// channelCmd 由 collection ID 或節目網址解析出節目資訊。
var channelCmd = &cobra.Command{
	Use:   "channel [id|url...]",
	Short: "解析節目 ID、名稱、正規網址與 RSS",
	Long: `由 collection ID 或 Apple Podcasts 節目網址解析出節目 ID、名稱、
正規網址與發佈者的 RSS feed。
預設每行一個 key: value 欄位、多筆之間以空行分隔；--json 一律輸出 JSON 陣列。
支援傳入多個目標或由 stdin 讀取。

範例:
  gx apple-podcast get channel 1702409419
  gx apple-podcast get channel 'https://podcasts.apple.com/tw/podcast/xxx/id1702409419' --json
  cat ids.txt | gx apple-podcast get channel --json`,
